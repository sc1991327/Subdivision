...
CCamera::GetRayDirection(int x, int y, CViewport& vp)
{
  ...
  double h = 2.f * nearDistance * tan((heightAngle/2.f)*(m_cam_PI/180.));
								 ...
								 CViewport& vp)
{
  ...
  double hOverD = tan((heightAngle/2.f)*(m_cam_PI/180.));
  double aspectRatio = vp.GetAspectRatio();
  if(aspectRatio < 1.f)
    hOverD *= aspectRatio;
...
CCamera::glDraw(CViewport& vp) const
{
  ...
  glRotated(radAngle*(360.f/m_cam_PI),rotAxis[0],rotAxis[1],rotAxis[2]);
